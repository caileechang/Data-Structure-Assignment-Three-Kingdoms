...
public class EF1_ExtraAlgorithm {

    public static void main(String[] args) {
        //create graph
        PathGraph graph = new PathGraph(10);
        graph.addEdge(1, 2);
        graph.addEdge(1, 3);
        graph.addEdge(1, 6);
        graph.addEdge(1, 10);
...
class PathGraph {
    ...
    public List<List<Integer>> findPaths(int source, int destination) {
        ...
        PriorityQueue<List<Integer>> queue = new PriorityQueue<>(Comparator.comparingInt(this::calculateCost));
    ...
    private int calculateCost(List<Integer> path) {
        // In A* search, the cost is the sum of the path length and a heuristic estimate of the remaining distance
        int pathLength = path.size() - 1;
        int remainingDistance = V - path.get(pathLength);
        return pathLength + remainingDistance;
    }
}
